...
We also needed to separate the app into different entry points, so webpack wouldn’t bundle any imports for one page into another. We then created entry points for each route, for example they would be pageOne.jsx, pageTwo.tsx, pageThree.tsx. The entry point would look something like this:
...
app.get('/', async (req, res, next) => {
   ...
   '../../public/assets/loadable-stats.json');
   let extractor;
...
description: 'In this blog post, we will talk through setting up loadable components in a SSR React app.',
date: '2021-01-05'
}
